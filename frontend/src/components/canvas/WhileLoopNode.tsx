/**
 * While Loop Node Component - Control flow node for conditional iteration
 */
...
// Default inputs/outputs for While Loop
const defaultInputs = [
  { name: 'trigger', type: 'trigger' as DataType },
  { name: 'condition', type: 'boolean' as DataType },
];
...
export const WhileLoopNode: React.FC<NodeProps<WhileLoopNodeData>> = ({
  data,
  selected,
  id,
}) => {
  ...
  // While Loop color (teal/cyan)
  const loopColor = '#16a085';
  ...
  return (
    ...
    >
                  ...
                  <span
                    className={`${
                      nodeStyles.fontSize === 'sm'
                        ...
                        ? 'text-xs'
                        : 'text-sm'
                    } ${input.name === 'condition' ? 'text-orange-400' : 'text-gray-300'}`}
                  >
                    {input.name}
                  </span>
                </div>
                  ...
                  <span
                    className={`${
                      nodeStyles.fontSize === 'sm'
                        ...
                        ? 'text-xs'
                        : 'text-sm'
                    } ${output.name === 'loop_body' ? 'text-teal-400 font-medium' : 'text-gray-300'}`}
                  >
                    {output.name}
                  </span>
                </div>
      ...
      {config && config.max_iterations !== undefined && (
        <div className="px-3 py-2 border-t border-darkborder">
          <div className="text-xs text-gray-500 flex justify-between">
            <span>max iterations:</span>
            <span className="text-teal-400 font-medium">{config.max_iterations}</span>
          </div>
        </div>
      )}
    </div>
